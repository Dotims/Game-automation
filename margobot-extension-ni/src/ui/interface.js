...
(function () {
  ...
  function createExpowiskaPanel() {
    ...
    testPotionsBtn.addEventListener("click", async () => {
      const targetAmount = parseInt(healPotionsInput.value) || 15;
      const multipliedTarget = targetAmount * 3; // stack = 3x
      ...
      console.log("[TEST POTEK] Wartość z inputa:", targetAmount);
      console.log("[TEST POTEK] Cel po mnożeniu (x3):", multipliedTarget);
      console.log("=================================================");

      try {
        if (typeof window.MargonemAPI?.testBuyPotionsAtHealer === 'function') {
          const result = await window.MargonemAPI.testBuyPotionsAtHealer(multipliedTarget);
          if (result) {
